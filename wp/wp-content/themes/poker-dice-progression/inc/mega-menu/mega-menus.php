...
if( ! class_exists( 'ProgressionFrontendWalker' ) ) {
	class ProgressionFrontendWalker extends Walker_Nav_Menu {
		...
		public function start_lvl( &$output, $depth = 0, $args = array() ) {
			...
			} elseif( $depth >= 1 && $this->menu_megamenu_status == "enabled" ) {
				$output .= "\n$indent<ul class=\"deep-level\">\n";
			} elseif( $depth ==0 && $this->menu_megamenu_status != "enabled" ) { 
				$output .= "\n$indent<ul class=\"menu-pro sub-menu\">\n";
			}
			else{
		...
		public function end_lvl( &$output, $depth = 0, $args = array() ) {
			...
			if( $depth === 0  && $this->menu_megamenu_status == "enabled" ) {
				...
				foreach($this->submenu_matrix as $row => $columns) {
					...
					$output = str_replace( "{current_row_".$row."}", "sf-mega-section columns-pro-".$this->allowed_columns, $output );
				}
			} else {
				$output .= "$indent</ul>\n";
			}
		...
		public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			if ( $this->menu_megamenu_status != "enabled" && ( ( strcasecmp( $item->attr_title, 'divider' ) == 0) ||
				...
				if( ($depth == 0 || $depth == 1) && $this->menu_megamenu_status != "enabled" ) {
					$class_names = $class_names ? ' class="normal-item-pro ' . $class_columns . ' ' . $class_names . '"' : '';
				}else{
    ...
    class ProCoreMegaMenus extends Walker_Nav_Menu {
		...
		function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			if ( ! empty( $item->_invalid ) ) {
				...
				$title = sprintf( esc_html__( '%s (Invalid)', 'poker-dice-progression' ), $item->title );
			} elseif ( isset( $item->post_status ) && 'draft' == $item->post_status ) {
				$classes[] = 'pending';
				/* translators: %s: title of menu item in draft status */
				$title = sprintf( esc_html__('%s (Pending)', 'poker-dice-progression'), $item->title );
			}
			...
			<li id="menu-item-<?php echo esc_attr($item_id); ?>" class="<?php echo implode(' ', $classes ); ?>">
				<dl class="menu-item-bar">
					<dt class="menu-item-handle">
						<span class="item-title"><span class="menu-item-title"><?php echo esc_html( $title ); ?></span> <span class="is-submenu" <?php echo esc_html( $submenu_text); ?>><?php esc_html_e( 'sub item', 'poker-dice-progression' ); ?></span></span>
						<span class="item-controls">
							...
							<span class="item-order hide-if-js">
								...
								?>" class="item-move-up"><abbr title="<?php esc_attr_e('Move up', 'poker-dice-progression'); ?>">&#8593;</abbr></a>
								|
								<a href="<?php
									echo wp_nonce_url(
										add_query_arg(
								...
								?>" class="item-move-down"><abbr title="<?php esc_attr_e('Move down', 'poker-dice-progression'); ?>">&#8595;</abbr></a>
							</span>
							<a class="item-edit" id="edit-<?php echo esc_attr($item_id); ?>" title="<?php esc_attr_e('Edit Menu Item', 'poker-dice-progression'); ?>" href="<?php
								echo ( isset( $_GET['edit-menu-item'] ) && $item_id == $_GET['edit-menu-item'] ) ? admin_url( 'nav-menus.php' ) : add_query_arg( 'edit-menu-item', $item_id, remove_query_arg( $removed_args, admin_url( 'nav-menus.php#menu-item-settings-' . $item_id ) ) );
							?>"><?php esc_html_e( 'Edit Menu Item', 'poker-dice-progression' ); ?></a>
						</span>
					</dt>
				</dl>
				...
				<div class="menu-item-settings" id="menu-item-settings-<?php echo esc_attr($item_id); ?>">
					<?php if( 'custom' == $item->type ) : ?>
						<p class="field-url description description-wide">
							<label for="edit-menu-item-url-<?php echo esc_attr($item_id); ?>">
								<?php esc_html_e( 'URL', 'poker-dice-progression' ); ?><br />
					...
					<p class="description description-thin">
						<label for="edit-menu-item-title-<?php echo esc_attr($item_id); ?>">
							<?php esc_html_e( 'Navigation Label', 'poker-dice-progression' ); ?><br />
							<input type="text" id="edit-menu-item-title-<?php echo esc_attr($item_id); ?>" class="widefat edit-menu-item-title" name="menu-item-title[<?php echo esc_attr($item_id); ?>]" value="<?php echo esc_attr( $item->title ); ?>" />
						</label>
					</p>
					<p class="description description-thin">
						<label for="edit-menu-item-attr-title-<?php echo esc_attr($item_id); ?>">
							<?php esc_html_e( 'Title Attribute', 'poker-dice-progression' ); ?><br />
					...
					<p class="field-link-target description">
						<label for="edit-menu-item-target-<?php echo esc_attr($item_id); ?>">
							...
							<?php esc_html_e( 'Open link in a new window/tab', 'poker-dice-progression' ); ?>
						</label>
					</p>
					<p class="field-css-classes description description-thin">
						<label for="edit-menu-item-classes-<?php echo esc_attr($item_id); ?>">
							<?php esc_html_e( 'CSS Classes (optional)', 'poker-dice-progression' ); ?><br />
							<input type="text" id="edit-menu-item-classes-<?php echo esc_attr($item_id); ?>" class="widefat code edit-menu-item-classes" name="menu-item-classes[<?php echo esc_attr($item_id); ?>]" value="<?php echo esc_attr( implode(' ', $item->classes ) ); ?>" />
						</label>
					</p>
					<p class="field-xfn description description-thin">
						<label for="edit-menu-item-xfn-<?php echo esc_attr($item_id); ?>">
							<?php esc_html_e( 'Link Relationship (XFN)', 'poker-dice-progression' ); ?><br />
							<input type="text" id="edit-menu-item-xfn-<?php echo esc_attr($item_id); ?>" class="widefat code edit-menu-item-xfn" name="menu-item-xfn[<?php echo esc_attr($item_id); ?>]" value="<?php echo esc_attr( $item->xfn ); ?>" />
						</label>
					</p>
					<p class="field-description description description-wide">
						<label for="edit-menu-item-description-<?php echo esc_attr($item_id); ?>">
							<?php esc_html_e( 'Description', 'poker-dice-progression' ); ?><br />
							<textarea id="edit-menu-item-description-<?php echo esc_attr($item_id); ?>" class="widefat edit-menu-item-description" rows="3" cols="20" name="menu-item-description[<?php echo esc_attr($item_id); ?>]"><?php echo esc_html( $item->description ); // textarea_escaped ?></textarea>
							<span class="description"><?php esc_html_e('The description will be displayed in the menu if the current theme supports it.', 'poker-dice-progression'); ?></span>
						</label>
					</p>
					...
					<div class="progression-mega-menu-options">
						...
						<p class="field-megamenu-notif description description-wide">
							<label for="edit-menu-item-megamenu-notif-<?php echo esc_attr($item_id); ?>">
								<?php esc_html_e( 'Menu Mini Banner Text', 'poker-dice-progression' ); ?>
						...
						<p class="field-megamenu-status description description-wide">
							<label for="edit-menu-item-megamenu-status-<?php echo esc_attr($item_id); ?>">
								<input type="checkbox" id="edit-menu-item-megamenu-status-<?php echo esc_attr($item_id); ?>" class="widefat code edit-menu-item-megamenu-status" name="menu-item-progression-megamenu-status[<?php echo esc_attr($item_id); ?>]" value="enabled" <?php checked( $item->progression_studios_megamenu_status, 'enabled' ); ?> />
								<strong><?php esc_html_e( 'Enable Mega Menu', 'poker-dice-progression' ); ?></strong>
							</label>
						</p>
						<p class="field-megamenu-columns description description-wide">
							<label for="edit-menu-item-megamenu-columns-<?php echo esc_attr($item_id); ?>">
								<?php esc_html_e( 'Mega Menu Number of Columns', 'poker-dice-progression' ); ?>
								<select id="edit-menu-item-megamenu-columns-<?php echo esc_attr($item_id); ?>" class="widefat code edit-menu-item-megamenu-columns" name="menu-item-progression-megamenu-columns[<?php echo esc_attr($item_id); ?>]">
									<option value="1" <?php selected( $item->progression_studios_megamenu_columns, '1' ); ?>><?php esc_html_e( '1 Column', 'poker-dice-progression' ); ?></option>
									<option value="2" <?php selected( $item->progression_studios_megamenu_columns, '2' ); ?>><?php esc_html_e( '2 Columns', 'poker-dice-progression' ); ?></option>
									<option value="3" <?php selected( $item->progression_studios_megamenu_columns, '3' ); ?>><?php esc_html_e( '3 Columns', 'poker-dice-progression' ); ?></option>
									<option value="4" <?php selected( $item->progression_studios_megamenu_columns, '4' ); ?>><?php esc_html_e( '4 Columns', 'poker-dice-progression' ); ?></option>
									<option value="5" <?php selected( $item->progression_studios_megamenu_columns, '5' ); ?>><?php esc_html_e( '5 Columns', 'poker-dice-progression' ); ?></option>
									<option value="6" <?php selected( $item->progression_studios_megamenu_columns, '6' ); ?>><?php esc_html_e( '6 Columns', 'poker-dice-progression' ); ?></option>
								</select>
							</label>
						</p>
						<p class="field-megamenu-title description description-wide">
							<label for="edit-menu-item-megamenu-title-<?php echo esc_attr($item_id); ?>">
								<input type="checkbox" id="edit-menu-item-megamenu-title-<?php echo esc_attr($item_id); ?>" class="widefat code edit-menu-item-megamenu-title" name="menu-item-progression-megamenu-title[<?php echo esc_attr($item_id); ?>]" value="disabled" <?php checked( $item->progression_studios_megamenu_title, 'disabled' ); ?> />
								<?php esc_html_e( 'Disable Mega Menu Column Title', 'poker-dice-progression' ); ?>
							</label>
						</p>
						
						<p class="field-megamenu-icon description description-wide">
							<label for="edit-menu-item-megamenu-icon-<?php echo esc_attr($item_id); ?>">
								<?php wp_kses( _e( 'Menu Icon (<a href="', 'poker-dice-progression' ) , true); ?>http://fortawesome.github.io/Font-Awesome/icons/" target="_blank">Font-Awesome Icon List</a>)
								<input type="text" id="edit-menu-item-megamenu-icon-<?php echo esc_attr($item_id); ?>" class="widefat code edit-menu-item-megamenu-icon" name="menu-item-progression-megamenu-icon[<?php echo esc_attr($item_id); ?>]" value="<?php echo esc_attr($item->progression_studios_megamenu_icon); ?>" />
							</label>
						</p>
						<p class="field-megamenu-widgetarea description description-wide">
							<label for="edit-menu-item-megamenu-widgetarea-<?php echo esc_attr($item_id); ?>">
								<?php esc_html_e( 'Mega Menu Widget Area', 'poker-dice-progression' ); ?>
								<select id="edit-menu-item-megamenu-widgetarea-<?php echo esc_attr($item_id); ?>" class="widefat code edit-menu-item-megamenu-widgetarea" name="menu-item-progression-megamenu-widgetarea[<?php echo esc_attr($item_id); ?>]">
									<option value="0"><?php esc_html_e( 'Select Widget Area', 'poker-dice-progression' ); ?></option>
									<?php
									if( ! empty( $wp_registered_sidebars ) && is_array( $wp_registered_sidebars ) ):
									foreach( $wp_registered_sidebars as $sidebar ):
									?>
									<option value="<?php echo esc_html($sidebar['id']); ?>" <?php selected( $item->progression_studios_megamenu_widgetarea, $sidebar['id'] ); ?>><?php echo esc_html($sidebar['name']); ?></option>
					...
					<p class="field-move hide-if-no-js description description-wide">
						<label>
							<span><?php esc_html_e( 'Move', 'poker-dice-progression' ); ?></span>
							<a href="#" class="menus-move-up"><?php esc_html_e( 'Up one', 'poker-dice-progression' ); ?></a>
							<a href="#" class="menus-move-down"><?php esc_html_e( 'Down one', 'poker-dice-progression' ); ?></a>
							<a href="#" class="menus-move-left"></a>
							<a href="#" class="menus-move-right"></a>
							<a href="#" class="menus-move-top"><?php esc_html_e( 'To the top', 'poker-dice-progression' ); ?></a>
						</label>
					</p>
					...
					<div class="menu-item-actions description-wide submitbox">
						<?php if( 'custom' != $item->type && $original_title !== false ) : ?>
							<p class="link-to-original">
								<?php printf( esc_html__('Original: %s', 'poker-dice-progression'), '<a href="' . esc_attr( $item->url ) . '">' . esc_html( $original_title ) . '</a>' ); ?>
							</p>
						<?php endif; ?>
						...
						); ?>"><?php esc_html_e( 'Remove', 'poker-dice-progression' ); ?></a> <span class="meta-sep hide-if-no-js"> | </span> <a class="item-cancel submitcancel hide-if-no-js" id="cancel-<?php echo esc_attr($item_id); ?>" href="<?php echo esc_url( add_query_arg( array( 'edit-menu-item' => $item_id, 'cancel' => time() ), admin_url( 'nav-menus.php' ) ) );
							?>#menu-item-settings-<?php echo esc_attr($item_id); ?>"><?php esc_html_e('Cancel', 'poker-dice-progression'); ?></a>
					</div>
					<div class="clear"></div>
